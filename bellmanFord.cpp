	...
	TC(t)
	{
		...
		for (int j = 0; j < e ; j++)
		{
			for (int i = 0; i < n; ++i)
			{
				for (auto l : graph[i]) {
					// cout << l.first << endl;
					int  temp = distance[l.first];

					if (distance[i] + l.second < distance[l.first])
						distance[l.first] = distance[i] + l.second;

					if (j == e - 1)
					{
						if (temp != distance[l.first])
						{
							flag = 1;
						}
					}
					// cout << i << " " << l.first << " " << l.second << endl;
				}
				// cout << endl;
			}
		}
